--------------------------------------------------
SELECT T.JRXKZH, T.NBJGH, T.KHTYBH, T.KHMC, T.ZJLB, T.ZJHM
FROM 
(
SELECT 
...
LEFT JOIN DCB_DCB_ORBRHA GL1
LEFT JOIN HRS_HRS_USRA01 D
LEFT JOIN 
(
     SELECT COD_VLU
...
ON H.COD_VLU = CG2.CERT_TYP
ON D.A0177 = CG2.CERT_NO AND D.HYREN_S_DATE <= '#{ETL_DT}' AND D.HYREN_E_DATE > '#{ETL_DT}'
ON CG1.OO_NO = GL1.ORGN_NUM AND GL1.HYREN_S_DATE <= '#{ETL_DT}' AND GL1.HYREN_E_DATE > '#{ETL_DT}'
ON CG1.CUST_NO = CG2.CUST_IDN
WHERE CG1.PABK_TYPE IN ('2','3','8') 
--------------------------------------------------