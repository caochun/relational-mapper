...
public class RelationalAlgebraInterpreter extends RelationalAlgebraBaseVisitor {
    ...
    public String visitCasestmt(CasestmtContext ctx){
        ...
        Object conditions = visit(ctx.conditions());
        Object attribute = visit(ctx.getChild(2));
        caseStmt.add("WHEN")
    ...
    @Override
    public String visitInnerjoin(InnerjoinContext ctx){
        StringJoiner subQuery = new StringJoiner(" ");
        Object lhs = visit(ctx.exp(0));
        Object innerJoinExp = visit(ctx.innerJoinExp());
        Object rhs = visit(ctx.exp(1));
        if (!isNested(ctx)) {
            subQuery.add("SELECT")
                    .add("*")
        ...
        subQuery.add(lhs.toString())
                .add(innerJoinExp.toString())
                .add(rhs.toString());
        if (ctx.innerJoinExp().conditions() != null) {
            Object conditions = visit(ctx.innerJoinExp().conditions());
            subQuery.add("ON")
                    .add(conditions.toString());
        }
        return subQuery.toString();
    ...
    @Override
    public String visitInnerJoinExp(InnerJoinExpContext ctx) {
        return "INNER JOIN";
    }
    ...
    public String visitCondition(ConditionContext ctx) {
        if(ctx.RELOP().getText().equals("LIKE"))
